...
class SimpleHT:
  ...
  # Print the keys of the hashtable
  ...
  # Corrupt the value of a given key
  def corrupt(self, key, value, ttl):
    if key in self.data:
        end = datetime.now() + timedelta(seconds = ttl)
        self.data[key] = (pickle.dumps(value), end)
    return True
...
def serve(port):
  ...
  file_server.register_function(sht.list_content)
  file_server.register_function(sht.print_content)
  file_server.register_function(sht.shutdown)
